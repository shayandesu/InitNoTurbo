...
class StableDiffusionInitNOPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    ...
    def fn_compute_loss(
        ...
        # print(f"self : {self_attn_loss.item():.10f} | cross : {cross_attn_loss.item():.10f}"
        #       f" | Clean Cross : {clean_cross_attention_loss.item():.10f}")
    ...
    def fn_initno(
        ...
        optimal_log_var, optimal_mu = None, None
        best_loss = 20
        for iteration in range(max_step):
            ...
            for i, t in enumerate(timesteps):
                scaled_latents = self.scheduler.scale_model_input(optimized_latents, t)
                ...
                if denoising_step_for_loss > 1:
                    with torch.no_grad():
                        ...
                        else: noise_pred_uncond = self.unet(scaled_latents, t, encoder_hidden_states=text_embeddings[0].unsqueeze(0)).sample
            ...
            with torch.cuda.amp.autocast(enabled=False):      # float32
                joint_loss.backward()
            ...
            while kld_loss > 0.001:
                ...
                optimizer.step()
                kld_loss = self.fn_calc_kld_loss_func(log_var, mu)
            # print(f"[KLD] Loss: {kld_loss.item():.2f}")
            
            if joint_loss < best_loss:
                best_loss = joint_loss
                optimal_log_var = log_var
                optimal_mu = mu

        optimized_latents = (latents * (torch.exp(0.5 * optimal_log_var)) + optimal_mu).clone().detach()
            ...
            with torch.enable_grad():
                ...
                for round in range(max_round):
                    optimized_latents, optimization_succeed, cross_self_attn_loss = self.fn_initno(
                        ...
                        denoising_step_for_loss=num_inference_steps,
                        device=device,
